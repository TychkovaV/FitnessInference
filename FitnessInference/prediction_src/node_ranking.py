...
class node_ranking(fitness_inference):
    ...
    def color_tree(self,nodes = None, method = 'mean_fitness',  offset=0.000, 
                   n_labels = 10, scramble=True, cmap = None, metadata={}):
        '''
        ...
        season_end = "05-31"
        if year is not None:
            end = datetime.fromisoformat(str(int(year)-1)+"-"+season_end)
        def reset_color(node):
        ...
        for ci, c in enumerate(nodes):
            ...
            c.color = [int(x*240) for x in 
                       np.array(cmap(int(255.0 * (n_nodes - ci)**1 / n_nodes**1))[:3])] if reset_color(c) == None else reset_color(c) # GREY FOR NEW
        ...
        for c in tree.get_nonterminals():
            ...
            for leaf in terms:
                if leaf.color:
                    ...
                    leaf_count+=1
            if leaf_count:
                c.color = (tmp_color/leaf_count).astype(int).tolist()
            else:
                c.color = (0,0,0)
