...
if hostname in ['rneher-iMac', 'ag-neher-xps13']:
    ...
    fasttree_bin = '/mnt/c/Users/Admin/Downloads/FastTree' #'/ebio/ag-neher/share/programs/bin/fasttree_cluster'
...
def calculate_tree(aln, outgroup, ancestral=True):
    ...
    with gzip.open(tmp_file_name, 'wt') as outfile:
    ...
    biopython_tree = Phylo.read(StringIO(fasttree_output.decode('ascii')), 'newick')
    biopython_tree.root.branch_length = 0.000001
    with gzip.open(tmp_file_name, 'rt') as infile:
...
def node_label_func(node, metadata={}):
    '''
    ...
    try:
        if len(node.name)<=3:
            return ""
        year = metadata['name'].split("-")[2][0:4] if 'name' in metadata.keys() else None 
        season_end = "05-31"
        end = datetime.fromisoformat(str(int(year)-2)+"-"+season_end)
        if 'name' not in metadata.keys() or "|" not in node.name:
            return None
        dates = [n.split("|")[1] for n in node.name.split("=")]
        for d in dates:
            if datetime.fromisoformat(d) < end:
                return node.name 
        return node.name + " *"# NODE_LABEL
    except:
        return None
...
def plot_prediction_tree(prediction, method='mean_fitness', internal = False, axes = None, cb = True, offset=0.0001, size=None, metadata={}):
    '''
    ...
    prediction.color_tree(method=method, offset = offset, n_labels = 1, cmap = my_favorite_cmap, nodes =nodes, metadata={})
    if not internal:
        prediction.interpolate_color()
    ax = draw_tree(prediction.T, axes = axes, cb=cb, size=size, metadata=metadata)


def draw_tree(tree, node_label = node_label_func, branch_label = lambda x:None,
                  cmap = cm.jet, axes = None, cb = True, size = None, metadata:dict = {}):
    '''
    plots a tree on an empty canvas including a scalebar of length 0.005
    '''
    import matplotlib.pyplot as plt
    from Bio import Phylo
    year = int(metadata['name'].split("-")[2][0:4]) if 'name' in metadata.keys() else None        
    if axes is None:
        fig = plt.figure(figsize = (100, size//2 if size!=None else 120)) #FIGSIZE
        axes = plt.subplot(111)
    Phylo.draw(tree, label_func = lambda x: node_label(x, metadata=metadata), 
               show_confidence = False,
               branch_labels = branch_label, axes=axes)
    axes.axis('off')
    xlimits = axes.get_xlim()
    ylimits = axes.get_ylim()
    x0 = xlimits[0]+(xlimits[1]-xlimits[0])*0.01
    x1 = x0+0.005
    y0 = ylimits[0]+(ylimits[1]-ylimits[0])*0.01
    #plt.plot([x0,x1], [y0,y0], lw=2, c='k')
    #plt.text(x0+0.0025, y0-(ylimits[1]-y0)*0.01, '0.005', ha='center') #GRID
    # fake a colorbar
    if cb:
        pass
        #sm = plt.cm.ScalarMappable(cmap=cmap, axes=axes, norm=plt.Normalize(vmin=0, vmax=100))
        #sm._A = []
        #cbar = plt.colorbar(mappable=sm, ticks=[0,100], shrink=0.5, aspect=10, pad=-0.05)
